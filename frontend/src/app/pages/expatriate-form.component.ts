        ...
        <mat-step [stepControl]="experienceForm" label="Experience">
          <form [formGroup]="experienceForm">
            <div class="step-content">
              ...
              <div class="form-group">
                <label>Summary of Professional Experience (20-1250 characters)</label>
                <textarea
                ...
                <div class="word-count">
                  {{ experienceForm.get('experienceSummary')?.value?.length || 0 }}/1250 characters
                </div>
                <div class="validation-errors" *ngIf="experienceForm.get('experienceSummary')?.touched && experienceForm.get('experienceSummary')?.errors">
                  ...
                  <span *ngIf="experienceForm.get('experienceSummary')?.errors?.['minlength']" class="error-text">
                    ❌ Minimum 20 characters required (current: {{ experienceForm.get('experienceSummary')?.value?.length || 0 }})
                  </span>
                  <span *ngIf="experienceForm.get('experienceSummary')?.errors?.['maxlength']" class="error-text">
                    ❌ Maximum 1250 characters allowed
                  </span>
                </div>
              </div>
...
export class ExpatriateFormComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Watch for country changes and update validators
    const countryControl = this.personalParticularsForm.get('country');
    if (countryControl) {
      countryControl.valueChanges.subscribe((country: string) => {
        if (country) {
          this.applyCountrySpecificValidators(country);
        }
      });
    }
  ...
  initializeForms(): void {
    ...
    this.experienceForm = this.fb.group({
      ...
      experienceSummary: ['', [Validators.required, Validators.minLength(20), Validators.maxLength(1250)]],
    });
  ...
  applyCountrySpecificValidators(country: string): void {
    ...
    if (idControl) {
      // For expatriates, allow any ID format - no strict validation
